...
namespace OpenWeb
{
	internal class HTTPSClientHello
	{
		...
		internal static readonly System.Collections.Generic.Dictionary<uint, string> dictTLSCipherSuites = new System.Collections.Generic.Dictionary<uint, string>
		{
		...
		public string ServerNameIndicator
		{
			get
			{
				...
				{
					result = this._ServerNameIndicator;
				}
				else
				{
					result = string.Empty;
		...
		private static string CipherSuitesToString(uint[] inArr)
		{
			...
			else
			{
				...
				{
					System.Text.StringBuilder stringBuilder = new System.Text.StringBuilder(inArr.Length * 20);
					for (int i = 0; i < inArr.Length; i++)
					{
						stringBuilder.Append("\t[" + inArr[i].ToString("X4") + "]\t");
						if ((ulong)inArr[i] < (ulong)((long)HTTPSClientHello.SSL3CipherSuites.Length))
						{
							stringBuilder.AppendLine(HTTPSClientHello.SSL3CipherSuites[(int)((uint)((System.UIntPtr)inArr[i]))]);
						}
						else
						{
		...
		public override string ToString()
		{
			...
			{
				stringBuilder.Append("A SSLv2-compatible ClientHello handshake was found. OpenWeb extracted the parameters below.\n\n");
			}
			else
			{
				stringBuilder.Append("A SSLv3-compatible ClientHello handshake was found. OpenWeb extracted the parameters below.\n\n");
			}
			...
			if (iExtType <= 13172)
			{
				...
				case 0:
				{
					...
					for (int i = 2; i < arrData.Length; i += 3 + num2)
					{
						...
						string @string = System.Text.Encoding.ASCII.GetString(arrData, i + 3, num2);
						if (num == 0)
						{
