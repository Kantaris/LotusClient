...
namespace OpenWeb
{
	internal class FTPGateway
	{
		public static void MakeFTPRequest(Session oSession, ref PipeReadBuffer buffBody, out HTTPResponseHeaders oRH)
		{
			...
			{
				throw new System.ArgumentException("Session missing Request objects.");
			}
			if (buffBody == null)
			{
				throw new System.ArgumentException("Response Stream may not be null.");
			}
			...
			else
			{
				...
				{
					ftpWebRequest.UseBinary = OpenWebApplication.Prefs.GetBoolPref("OpenWeb.ftp.UseBinary", true);
				}
			}
			if (!string.IsNullOrEmpty(oSession.oRequest.headers.UriUserInfo))
			{
				string text = Utilities.TrimAfter(oSession.oRequest.headers.UriUserInfo, '@');
			...
			else
			{
				if (oSession.oRequest.headers.ExistsAndContains("Authorization", "Basic "))
				{
					...
					text2 = System.Text.Encoding.UTF8.GetString(System.Convert.FromBase64String(text2));
				...
				else
				{
					...
					{
						if (OpenWebApplication.Prefs.GetBoolPref("OpenWeb.ftp.AlwaysDemandCredentials", false))
						{
							byte[] bytes = System.Text.Encoding.UTF8.GetBytes("Please provide login credentials for this FTP server".PadRight(512, ' '));
			...
			ftpWebRequest.UsePassive = OpenWebApplication.Prefs.GetBoolPref("OpenWeb.ftp.UsePassive", true);
			...
			catch (WebException ex)
			{
				...
				if (ftpWebResponse2 != null)
				{
					...
					{
						bytes2 = System.Text.Encoding.UTF8.GetBytes("This FTP server requires login credentials".PadRight(512, ' '));
					...
					bytes2 = System.Text.Encoding.UTF8.GetBytes(string.Format("{0}{1}{2}", "OpenWeb was unable to act as a HTTP-to-FTP gateway for this response. ", ftpWebResponse2.StatusDescription, string.Empty.PadRight(512, ' ')));
					buffBody.Write(bytes2, 0, bytes2.Length);
				}
				else
				{
					byte[] bytes2 = System.Text.Encoding.UTF8.GetBytes(string.Format("{0}{1}{2}", "OpenWeb was unable to act as a HTTP-to-FTP gateway for this response. ", ex.Message, string.Empty.PadRight(512, ' ')));
					buffBody.Write(bytes2, 0, bytes2.Length);
				}
				oRH = new HTTPResponseHeaders();
